...
def load_data(dataset_str):
    ...
    if dataset_str != 'flickr':
        ...
        adj = sp.load_npz(os.path.join('flickr/data/raw','adj_full.npz'))
        ...
        idx_val = range(len(idx_train),len(idx_train)+5000)
        idx_test = list(range(len(idx_train)+len(idx_val),len(idx_train)+len(idx_val)+5000))
        train_mask = sample_mask(idx_train, labels.shape[0])
        val_mask = sample_mask(idx_val, labels.shape[0])
        test_mask = sample_mask(idx_test, labels.shape[0])
        y_train = np.zeros(labels.shape)
